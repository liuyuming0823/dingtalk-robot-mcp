...
class DingTalkMCPServer {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        if (name === 'send_dingtalk_single_message') {
          const { userId, userIds, userName, content } = args as {
            userId?: string;
            userIds?: string[];
            userName?: string;
            content: string;
        ...
        if (name === 'send_dingtalk_group_message') {
          const { chatId, content } = args as { chatId: string; content: string };
  ...
  async run(): Promise<void> {
    ...
    if (!appKey || !appSecret) {
      console.error(
        'Error: DINGTALK_APP_KEY and DINGTALK_APP_SECRET environment variables are required'
      );
      process.exit(1);
    }
